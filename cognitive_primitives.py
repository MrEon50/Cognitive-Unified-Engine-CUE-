...
class CognitiveObject(nn.Module):
    ...
    Can be a single state vector or a batch of representations [B, L, D].
    """
    def __init__(self, state_tensor, name="CognitiveObject"):
        super().__init__()
        self.name = name
...
class CognitiveRelation(CognitiveObject):
    ...
    """
    def __init__(self, obj_a: CognitiveObject, obj_b: CognitiveObject, rel_type="Relation"):
        # Relational state combination: mean vector + difference embedding
        state_a = obj_a.state
        state_b = obj_b.state
...
class CognitivePalette(nn.Module):
    ...
    def difference_D(self, state_a, state_b):
        """D (Delta): Computes vector difference state_b - state_a."""
        return state_b - state_a

    def metric_M(self, state_a, state_b):
        """M (Metric): Computes Euclidean distance between states."""
        return torch.norm(state_b - state_a, dim=-1, keepdim=True)

    def equivalence_E(self, state_a, state_b):
        """E (Equivalence/Symmetry): Exponential similarity in [0, 1]."""
        dist = self.metric_M(state_a, state_b)
        return torch.exp(-torch.abs(self.gamma) * dist)

    def direction_V(self, state_a, state_b):
        """V (Vector): Directional unit vector from state_a to state_b."""
        diff = self.difference_D(state_a, state_b)
        norm = torch.norm(diff, dim=-1, keepdim=True) + 1e-8
        return diff / norm

    def transform_T(self, state, action_vector, tau=1.0):
        """T (Transformation): State transition over time step tau."""
        return state + action_vector * tau

    def causality_C(self, state_a, state_b):
        """C (Causality): Evaluates causal link score between states."""
        concat = torch.cat([state_a, state_b], dim=-1)
        return torch.sigmoid(self.causal_weight(concat))

    def goal_gravity_G(self, current_state, goal_state):
    ...
    def forward(self, state_a, state_b, goal_state=None):
        """
